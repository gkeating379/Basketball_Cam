...
def draw_k_best_lines(image, add_k, court, court_canny):
    ...
    if lines is not None:
        ...
        H = find_best_projection(image, verts, horzs, court, court_canny)
...
def get_best_frame_homography(frame, court, court_canny):
    ...
    lines, H = draw_k_best_lines(kmeans, 10, court, court_canny)
...
court_canny = cv2.imread('court_boundary.png')
while cap.isOpened():
    # Capture frame-by-frame
    ret, frame = cap.read()

    # if frame is read correctly and not last ret is True
    if not ret:
        break

    start_time = time.time()
    H = get_best_frame_homography(frame)    

    #draw projection onto the court
    if H is not None:
        img_proj = cv2.warpPerspective(frame, H, (court.shape[1], court.shape[0]))
        proj_on_court = np.where(img_proj == 0, court, img_proj)
        cv2.imwrite('test2.png', proj_on_court)
    ...
    print(f'Frame Runtime: {t} seconds')
